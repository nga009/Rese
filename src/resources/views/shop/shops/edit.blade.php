...
@section('title', '店舗情報編集 - Rese')

@section('css')
<link rel="stylesheet" href="{{ asset('css/shop/common.css')}}">
<link rel="stylesheet" href="{{ asset('css/shop/shops/edit.css')}}">
@endsection
            ...
            <div class="card-body">
                @if ($shop->shop_image)
                    <div class="current-image">
                        <p>現在の画像:</p>
                        <img src="{{ asset('storage/' . $shop->shop_image) }}" alt="{{ $shop->name }}">
                    </div>
                @endif

                <form method="POST" action="{{ route('shop.shops.update', $shop) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
